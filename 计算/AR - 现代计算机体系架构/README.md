...
先从 [[QUESTIONS|推免面试回忆题]] 识别自己是在 ISA、数据通路、存储层次还是并行机制上断链，再回到对应章节追踪状态和数据流。复答性能题时统一使用执行时间、CPI、AMAT、吞吐率与能耗等量尺，并说明优化改变了哪一项、代价落在何处。

### 08/24—09/03 冲刺安排

具体时间轴见 [[推免复习总览#08/24—09/03 冲刺计划|总计划]]。

+ **首次闭卷**：08/27 完成 Q01—Q11 全量快筛，并与 DL、OS 共同画出 ISA—数据通路—流水线—Cache—虚拟内存—I/O 主线。
+ **集中复答**：09/01 复答全部红黄题；09/02 的最终模拟中至少抽取一道处理器题和一道存储层次题。
+ **最低完成标准**：能区分 ISA、微体系结构与操作系统接口，并用 CPI、AMAT、吞吐率或能耗说明性能改进及其代价。

...
先从 [[QUESTIONS|推免面试回忆题]] 判断自己混淆的是任务、模型、损失、训练算法还是评价指标，再回到统计学习或深度模型章节补齐机制。复答时写出关键目标函数或计算流程，说明数据假设、泛化风险和评价边界，并把视觉、序列、图与生成任务放回各自的数据结构中比较。

### 08/24—09/03 冲刺安排

具体时间轴见 [[推免复习总览#08/24—09/03 冲刺计划|总计划]]。

+ **首次闭卷**：08/29 完成 Q01—Q18 全量快筛，按任务、表示、模型、损失、训练与评价串联统计学习和深度模型。
+ **集中复答**：09/01 与 VI 一起复答全部红黄题；09/02 只保留与个人项目或目标方向直接相关的模型追问。
+ **最低完成标准**：每题能写出关键计算或目标函数，说明数据假设与泛化风险，并区分视觉、序列、图和生成任务的输入结构与评价指标。

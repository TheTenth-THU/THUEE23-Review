...
先做 [[QUESTIONS|推免面试回忆题]]，再判断问题需要解析函数的局部结构、围道积分与留数、变换域化简，还是偏微分方程的定解框架。复答时先说明解析性、区域、初边值条件和适定性，再选择留数、特征线、分离变量、基本解或 Green 函数，避免只背求解公式。

### 08/24—09/03 冲刺安排

具体时间轴见 [[推免复习总览#08/24—09/03 冲刺计划|总计划]]。

+ **首次闭卷**：08/28 完成现有全部原题，并只回到直接关联的解析性、留数或偏微分方程定解章节。
+ **集中复答**：09/01 只复答仍为红色的题，不扩张到完整课程；09/02 不再新增内容。
+ **最低完成标准**：回答先声明区域、解析性、初边值条件与适定性，再说明所选方法为什么适用及其边界。
